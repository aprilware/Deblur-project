...
/// when it's BELOW 0.30, otherwise <see cref="Visibility.Collapsed"/>. Used to show
...
public sealed class LowConfidenceToVisibilityConverter : IValueConverter
{
    ...
    {
        if (value is float f && f < 0.30f) return Visibility.Visible;
        if (value is double d && d < 0.30) return Visibility.Visible;
